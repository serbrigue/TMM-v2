...
const NewsletterModal = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setStatus('loading');
        try {
            await axios.post('http://localhost:8000/api/newsletter/subscribe/', { email });
            setStatus('success');
            localStorage.setItem('newsletter_modal_seen', 'true');
            setTimeout(() => {
                setIsOpen(false);
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm transition-opacity duration-300">
            <div className="bg-white rounded-3xl shadow-2xl max-w-4xl w-full overflow-hidden flex flex-col md:flex-row relative animate-fade-in-up">
                ...
                {/* Image Side */}
                <div className="md:w-1/2 bg-brand-pink relative overflow-hidden">
                    <div className="absolute inset-0 bg-gradient-to-br from-brand-pink/80 to-brand-calypso/80 mix-blend-multiply" />
                    <img
                    ...
                    <div className="absolute inset-0 flex flex-col justify-end p-8 text-white">
                        ...
                        <h3 className="text-3xl font-heading font-bold mb-2">Guía de Manualidades 2025</h3>
                ...
                <div className="md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
                    {status === 'success' ? (
                        <div className="text-center py-8">
                            ...
                            <h3 className="text-2xl font-bold text-gray-800 mb-2">¡Gracias por suscribirte!</h3>
                            <p className="text-gray-600">Revisa tu correo, te hemos enviado el enlace de descarga.</p>
                        </div>
                    ) : (
                        <>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1 ml-1">Tu mejor correo</label>
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            className="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-brand-pink focus:border-transparent transition-all"
                                        />
                                    </div>
                                </div>
                                ...
                                <button
                                    ...
                                    className="w-full bg-brand-calypso text-white font-bold py-4 rounded-xl hover:bg-opacity-90 transition-all transform hover:-translate-y-1 shadow-lg hover:shadow-brand-calypso/30 flex items-center justify-center gap-2"
                                >
